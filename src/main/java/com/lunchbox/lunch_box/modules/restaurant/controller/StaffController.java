...
import com.lunchbox.lunch_box.modules.restaurant.dto.request.UpdateStaffActiveRequest;
...
public class StaffController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<StaffResponse>> toggleStaffActive(
            @PathVariable Long id,
            @Valid @RequestBody UpdateStaffActiveRequest request) {
        StaffResponse staff = staffService.toggleStaffActive(id, request.getIsActive());
        return ResponseEntity.ok(ApiResponse.success(HttpStatus.OK.value(), "Staff active status updated successfully", staff));
    }
